function loadSVG() {
  fetch("assets/WorldMap.svg")
    ...
    .then((svgText) => {
      ...
      const clickedCountries =
        JSON.parse(sessionStorage.getItem("clickedCountries")) || [];
      ...
      document.querySelectorAll(".landxx").forEach(function (element) {
        element.addEventListener("click", function () {
          document.querySelectorAll(".landxx").forEach((remove) => {
          ...
          if (countryId) {
            if (!clickedCountries.includes(countryId)) {
              ...
              sessionStorage.setItem(
                "clickedCountries",
                JSON.stringify(clickedCountries)
              );
            }
