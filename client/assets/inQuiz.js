...
form.addEventListener("submit", async function (e) {
  ...
  if (index + 1 !== questions.length) {
    ...
    await saveResults(score, 3, Math.floor(score / questions.length)); // Change conuntry code 2
...
const fetchQuestions = async () => {
  const response = await fetch("http://localhost:3000/quiz-questions/3"); // Change conuntry code 2
